...
def stack(arrs, axis=0):
    return tf.stack(values=arrs, axis=axis)
...
def kl_divergence(p, q):
    ...
    return tf.reduce_sum(condition(p) * tf.log(condition(p)/condition(q)), axis=-1)


def h_distance(p, q):
    # compute Hellinger distance
    return tf.multiply(tf.square(tf.reduce_sum(tf.square(tf.sqrt(p)-tf.sqrt(q)))), 1/np.sqrt(2))

def sl_distance(p, q):
    return tf.reduce_sum(tf.square(p - q), axis=1)

# assume they are distributions before.
def ce_distance(p, q):
    logq =  tf.log(q + 1e-8)
    cross_entropy = -tf.reduce_sum(tf.multiply(logq, p), axis=-1)
...
def compute_belief_and_e_distance(e, b, dis_type):
    distance_fn = get_distance_fn(dis_type=dis_type)
    if dis_type.lower() == 'sl' or dis_type.lower() == 'ce':
        return distance_fn(e, b)
    else:
        return distance_fn(tf.nn.softmax(e), tf.nn.softmax(b))


def condition(x, eps=1e-5):
    # smooth e,b when obtaining r_c
    return tf.add(eps, tf.multiply(1.0 - 2.0 * eps, x))
